package com.rishi.androicomponents.presentation.screenD
...
fun ScreenDContent(
    ...
    Column(
        ...
        Text(
            text = "YO! I am Screen D",
            fontSize = 22.sp,
            fontWeight = FontWeight.Bold
        )
        Spacer(Modifier.size(24.dp))
        Button(onClick = {
            onEvent(ScreenDUiEvent.GoBack)
        }) {
...
private fun handleSideEffects(
    ...
    when (effect) {
        is ScreenDSideEffect.BackToScreenC -> {
            onAction(ScreenDActions.BackToScreenC(effect.id, effect.name))
        }
